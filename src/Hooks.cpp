...
void Hooks::Install()
{
	...
	REL::Relocation<std::uintptr_t> vtbl1{ RE::Character::VTABLE[0] };
	_UpdateCharacter = vtbl1.write_vfunc(offset, UpdateCharacter);

	REL::Relocation<std::uintptr_t> vtbl2{ RE::PlayerCharacter::VTABLE[0] };
	_UpdatePlayer = vtbl2.write_vfunc(offset, UpdatePlayer);
}
...
void Hooks::UpdateCharacter(RE::Actor* a_actor, float a_delta)
{
	...
	if (auto thread = ActorManager::GetActorThread(a_actor, false))
		thread->Update();
}